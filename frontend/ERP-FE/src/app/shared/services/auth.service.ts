...
import { decodeJwtClaims, normalizeUserFromClaims } from '../../core/utils/jwt.utils';
...
export class AuthService {
  ...
  loadConnectedUser(): Observable<AuthUserState> {
    ...
    return this.userIamService.getMe().pipe(
      ...
      catchError((error) => {
        // Si l'API /User/Users fail (403, 500, etc), utiliser le fallback du token JWT
        if (!accessToken) {
  ...
  private normalizeRole(role: string): string {
    ...
    if (normalized === 'directeur general' || normalized === 'directeur generale' || normalized === 'dg') {
  ...
  /**
   * Retourne la liste des rôles de l'utilisateur courant (chaîne normalisée)
   */
  getUserRoles(): string[] {
    return (this.currentUserValue?.roles ?? []).map((role) => this.normalizeRole(role));
  }
}
